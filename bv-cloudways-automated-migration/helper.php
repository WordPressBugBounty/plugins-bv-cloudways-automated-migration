...
if (!class_exists('CWSHelper')) :
	class CWSHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!CWSWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = CWSWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					CWSWPFileSystem::getInstance()->putContents($fname, $modified_content,
							CWSWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
