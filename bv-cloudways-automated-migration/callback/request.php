...
if (!class_exists('CWSCallbackRequest')) :
	class CWSCallbackRequest {
		public $params;
		public $method;
		public $wing;
		public $is_afterload;
			...
			if (array_key_exists('bvprms', $in_params) && isset($in_params['bvprms'])) {
				...
				if ($this->compare_mac($this->bvprmsmac, $calculated_mac) === true) {
					...
					if (array_key_exists('sersafe', $in_params)) {
						...
						$in_params[$key] = CWSCallbackRequest::serialization_safe_decode($in_params[$key]);
					}
		...
		public static function serialization_safe_decode($data) {
			if (is_array($data)) {
				$data = array_map(array('CWSCallbackRequest', 'serialization_safe_decode'), $data);
		...
		public function verify($data, $sig, $sighshalgo) {
			...
			$public_key_str = CWSWPFileSystem::getInstance()->getContents($key_file);
