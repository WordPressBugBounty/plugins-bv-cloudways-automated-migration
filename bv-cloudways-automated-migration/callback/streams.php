...
if (!class_exists('CWSRespStream')) :

	class CWSStream extends CWSCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new CWSRespStream($request);
			if ($request->isAPICall()) {
				$stream = new CWSHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class CWSRespStream extends CWSStream {
	public $bvboundry;
...
class CWSHttpStream extends CWSStream {
	var $user_agent = 'CWSHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
